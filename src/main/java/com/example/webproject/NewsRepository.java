...
//
    @Query(value = "select ROW_NUMBER() OVER(ORDER BY n.input_date) AS id, n.news_date AS news_date, nc.category AS category, n.title AS title,n.link, n.press_agency AS press_agency ,nc3.keyword AS keyword ,n.input_date AS input_date\n" +
            ...
            "\tfrom news_category nc ,(select * from news_category nc2 where (keyword like '%' || text(:search) || '%' and text(:distinct) = '')) as nc2\n" +
    ...
    Page<News> findNewsSearch(@Param("fromDate") String fromDate, @Param("toDate") String toDate, @Param("firstcolName") String category, @Param("secondcolName") String distinct, @Param("searchText") String search, Pageable pageable);
//
//
//    @Query(value = "select ROW_NUMBER() OVER(ORDER BY n.input_date) AS id, n.news_date AS news_date, nc.category AS category, n.title AS title,n.link, n.press_agency AS press_agency ,nc3.keyword AS keyword ,n.input_date AS input_date\n" +
//            "from news n ,news_category nc ,news_list nl , \n" +
